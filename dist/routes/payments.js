...
paymentsRouter.post('/orders', authenticate, asyncHandler(async (req, res) => {
    ...
    const order = await razorpayService.createOrder(payload.amount, payload.currency, undefined, payload.metadata);
    res.json({
        data: {
            orderId: order.id,
            amount: order.amount,
...
// Verify payment
paymentsRouter.post('/verify', authenticate, asyncHandler(async (req, res) => {
    ...
    const isValid = razorpayService.verifyPayment(payload.orderId, payload.paymentId, payload.signature, keySecret);
    if (isValid) {
